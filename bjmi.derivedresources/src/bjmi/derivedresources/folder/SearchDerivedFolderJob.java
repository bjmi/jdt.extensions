    ...
    aSyncExec(new Runnable() {
      ...
      public void run() {
        ...
        try {
          ...
          Logging.log(DerivedResourcePlugin.getInstance(), new Status(IStatus.ERROR, DerivedResourcePlugin.PLUGIN_ID,
              "Command execution failed", exc));
        } catch (final NotDefinedException exc) {
          Logging.log(DerivedResourcePlugin.getInstance(), new Status(IStatus.ERROR, DerivedResourcePlugin.PLUGIN_ID,
              "Command not defined", exc));
        } catch (final NotEnabledException exc) {
          Logging.log(DerivedResourcePlugin.getInstance(), new Status(IStatus.ERROR, DerivedResourcePlugin.PLUGIN_ID,
              "Command not enabled", exc));
        } catch (final NotHandledException exc) {
          Logging.log(DerivedResourcePlugin.getInstance(), new Status(IStatus.ERROR, DerivedResourcePlugin.PLUGIN_ID,
              "Command not handled", exc));
        }
